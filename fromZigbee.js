...
const converters = {
    ...
    generic_time_dev_change: {
        cid: 'genTime',
        type: 'devChange',
        convert: (model, msg, publish, options) => {
            const result = {};
            if (typeof msg.data.data['time'] == 'number') {
                result.time = msg.data.data['time'];
            }
            if (typeof msg.data.data['timeStatus'] == 'number') {
                result.time_status = msg.data.data['timeStatus'];
            }
            if (typeof msg.data.data['timeZone'] == 'number') {
                result.time_zone = msg.data.data['timeZone'];
            }
            return result;
        },
    },
